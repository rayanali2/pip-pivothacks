...
These are generated by running the API in MOCK_MODE (`npm run fixtures`, without `ANTHROPIC_API_KEY`) and saved under `ios/Pip/Pip/Resources/Offline/`. The app uses them only when the API is unreachable. `capture_voice.json` and `rerank_25.json` include `pipeline`; iOS decodes it as optional.
