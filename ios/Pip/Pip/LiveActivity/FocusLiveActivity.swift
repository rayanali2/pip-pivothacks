...
enum FocusLiveActivity {
    ...
    /// still reachable through `Activity<PipFocusAttributes>.activities`.
    private static var current: Activity<PipFocusAttributes>?
    ...
    static func start(title: String, endsAt: Date, nextLabel: String?, moneyAtRisk: Double?) {
        ...
        let attributes = PipFocusAttributes(title: title, nextLabel: nextLabel, moneyAtRisk: moneyAtRisk)
        let state = PipFocusAttributes.ContentState(endsAt: endsAt, phase: "running", extendedMinutes: 0)
        do {
            current = try Activity<PipFocusAttributes>.request(
    ...
    static func update(endsAt: Date, phase: String, extendedMinutes: Int) {
        ...
        let state = PipFocusAttributes.ContentState(endsAt: endsAt, phase: phase, extendedMinutes: extendedMinutes)
    ...
    private static func endAll(phase: String) {
        var activities = Activity<PipFocusAttributes>.activities
    ...
    private static func content(for state: PipFocusAttributes.ContentState) -> ActivityContent<PipFocusAttributes.ContentState> {
